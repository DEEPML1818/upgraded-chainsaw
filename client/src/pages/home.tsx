...
const LazyFramerCodingBackground = lazy(() => 
  import('@/components/framer-coding-background').then(module => ({ default: module.FramerCodingBackground }))
);
const LazyDynamicCharts = lazy(() => 
  import('@/components/dynamic-charts').then(module => ({ default: module.DynamicCharts }))
);
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Initial check
    const initialMobile = window.innerWidth <= 768 || /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  ...
  return (
    <div id="smooth-wrapper" className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 relative overflow-hidden flex flex-col">
      ...
      <ErrorBoundary>
        <div id="smooth-content" className="relative z-20 flex-1">
          ...
          <div className="liquid-section slide-in-left">
            {!isMobile ? (
              <Suspense fallback={<div className="h-96 bg-slate-800/50 rounded-lg animate-pulse"></div>}>
                <LazyDynamicCharts />
              </Suspense>
            ) : (
              <DynamicCharts />
